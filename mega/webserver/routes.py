...
@routes.get("/WhitE_DeviL09/{message_id}/{file_name}")
...
@routes.get("/channel/{chat_id}/{message_id}")
async def stream_s_handler(request):
    try:
        message_id = int(request.match_info['message_id'])
        chat_id = request.match_info['chat_id']
        return await media_group_streamer(request, message_id, int(f"-100{chat_id}"))
    except ValueError as e:
        logging.error(e)
        raise web.HTTPNotFound


async def media_group_streamer(request, message_id: int, chat_id: int):
    range_header = request.headers.get('Range', 0)
    media_msg = await MegaDLBot.get_messages(chat_id, message_id)
